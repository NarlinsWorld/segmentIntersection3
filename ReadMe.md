...
Given two lines, $$X=P+t\cdot\mathbf{u}$$ and $$X=Q+s\cdot\mathbf{v},$$
...
<p>
Of course, we cannot cancel out the two vectors, $$\mathbf{u^{\perp}},$$ since that would lead to an inadmissible vector division, but the dot product, $$\mathbf{v}\cdot\mathbf{u}^{\perp}$$ is a scalar. Finally, we can substitute into [eq:Perp2] to get the intersection.
$$
